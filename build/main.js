...
var import_cupra_client = require("./cupra-client");
var import_state_manager = require("./state-manager");
class CupraConnect extends utils.Adapter {
  pollInterval = null;
  cupraClient = null;
  stateManager = new import_state_manager.StateManager(this);
  constructor(options = {}) {
    super({
      ...options,
      name: "cupra-connect"
  ...
  async onReady() {
    ...
    if (accessTokenValue && accessTokenValue.val) {
      const info = JSON.parse(accessTokenValue.val);
      this.cupraClient = new import_cupra_client.CupraClient(info.refresh_token);
    } else {
      this.cupraClient = new import_cupra_client.CupraClient(this.config.initialRefreshToken);
    }
...
if (require.main !== module) {
  module.exports = (options) => new CupraConnect(options);
} else {
  (() => new CupraConnect())();
}
